...
class Statistics():
    ...
    def Describe(self, files, cat):
        ...
        for it in list(product(c, files.sscat, self.subcat, self.sort)):
            ...
            if os.path.exists(f'{files.input}/{it[0]}_{it[1]}_{self.table}.csv'):
                ...
                nanIndex = self.df.index[self.df.isnull().any(axis = 1)]
                self.df = self.df.drop(nanIndex, 0, inplace = True)
                ...
                if names['subcat'] == 'all':
                    ...
                    s = names['sort']
                    #if s.startswith('sliced'):
                    #    print(names['id'], names['sort'], names['subcat'])
    ...
    def ConcatTables(self, files, name, tables, save, sort = None, values = None):

        df = pd.DataFrame()

        for table in tables:
            table = self.Load(f'{files.input}/{table}.csv')
            df = pd.concat([df, table], ignore_index = True)

        if sort != None:
            df = self.SortDataFrameInner(df, sort, values)
        if save == True:
            df.to_csv(f'{files.output}/{name}.csv', sep = ';', index = None)
    ...
    def CombineTables(self, files, name, table1, table2, how, save, key, *fields):
        ...
        if save == True:
            table1.to_csv(f'{files.output}/{name}.csv', sep = ';', index = None)
        return table1
    ...
    def SortCategories(self, files):
        ...
        self.cat = list(set(self.resume['cat']))
        self.sort = list(set(self.resume['sort']))
        
        for it in list(product(self.cat, self.sort)):
        
            cat = it[0]
            sort = it[1]

            try:
                all = self.resume.loc[f'{cat}_all_all', '%catArea']
                ...
                print(self.resume.loc[f'{cat}_Olivine_all']/self.resume.loc[f'{cat}_all_all', '%catArea'], self.samples.loc[cat, 'lithologyDef'])
            except:
                pass
            ...
            if os.path.exists(f'{files.input}/{cat}_{ssc}_{self.table}.csv'):
                ...
                for i in range(1, len(values)):
                    ...
                    if d.shape[0] > 0:
                        ...
                        self.valuesSlices.append((m+M)/2)
                        dd = pd.concat([dd, d])
                        self.valuesSlices = list(set(self.valuesSlices))
