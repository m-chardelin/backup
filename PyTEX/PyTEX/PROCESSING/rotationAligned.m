function [ebsdAligned] = rotationAligned(ebsd)    

		%%
		%pf_grid=S2Grid('EQUISPACED','HEMISPHERE','lower', 'resolution',res*degree);
		pf_grid = equispacedS2Grid('resolution',1*degree);
		%%
		odf_olivine = calcDensity(ebsd('Forsterite').orientations);

		%%
		pf_100 = calcPDF(odf_olivine,Miller(1,0,0, ebsd('Forsterite').CS,'hkl'),pf_grid);
		pf_010 = calcPDF(odf_olivine,Miller(0,1,0, ebsd('Forsterite').CS,'hkl'),pf_grid);
		pf_001 = calcPDF(odf_olivine,Miller(0,0,1, ebsd('Forsterite').CS,'hkl'),pf_grid);
		...
		Ys_in_Structural = vector3d(0,1,0);
		Zs_in_Structural = vector3d(0,0,1);
		% use map option to define rotation SEM to Structural frame
		rot = rotation('map',Xs_in_SEM,Zs_in_Structural,Ys_in_SEM,Xs_in_Structural);
